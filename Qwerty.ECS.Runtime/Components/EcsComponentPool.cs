...
namespace Qwerty.ECS.Runtime.Components
{
	internal unsafe class EcsComponentPool<T> : IEcsComponentPool where T : struct, IEcsComponent 
	{
		...
		public EcsComponentPool(int capacity)
		{
			...
			m_buckets->SetCount<UnsafeArray>(capacity);
		...
		public void Write(int entityIndex, in T component)
		{
			...
			if (bucketIndex >= m_buckets->CapacityInBytes / MemoryUtilities.SizeOf<UnsafeArray>())
			{
			...
			if (bucket.Ptr == null)
			{
				...
				newBucket->SetCount<T>(BucketSize);
		...
		public void Dispose()
		{
			int capacity = m_buckets->CapacityInBytes / MemoryUtilities.SizeOf<UnsafeArray>();

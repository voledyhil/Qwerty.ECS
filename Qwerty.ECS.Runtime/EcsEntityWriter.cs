...
namespace Qwerty.ECS.Runtime
{
        ...
        where T7 : struct, IEcsComponent
    {
        ...
        public readonly unsafe struct EcsEntityWriter<T0, T1, T2, T3, T4, T5, T6, T7, T8> : IDisposable
        where T0 : struct, IEcsComponent
        where T1 : struct, IEcsComponent
        where T2 : struct, IEcsComponent
        where T3 : struct, IEcsComponent
        ...
        where T8 : struct, IEcsComponent
    {
        ...
        private readonly int m_offsetT6;
        private readonly int m_offsetT7;
        private readonly int m_offsetT8;
        private readonly int m_entitySizeOf;
        
        private readonly IntPtr m_body;
        private readonly int* m_len;

        internal EcsEntityWriter(int capacity, int rowSizeInBytes, int offsetT0, int offsetT1, int offsetT2, int offsetT3, int offsetT4, int offsetT5, int offsetT6, int offsetT7, int offsetT8)
        {
            m_capacity = capacity;
            m_rowSizeInBytes = rowSizeInBytes;
            m_offsetT0 = offsetT0;
        ...
        public void Write(T0 comp0, T1 comp1, T2 comp2, T3 comp3, T4 comp4, T5 comp5, T6 comp6, T7 comp7, T8 comp8)
        {
            int len = (*m_len)++;
            if (len >= m_capacity)
            {
            ...
            MemoryUtil.Write(m_body, len * m_rowSizeInBytes + m_offsetT6, comp6);
            MemoryUtil.Write(m_body, len * m_rowSizeInBytes + m_offsetT7, comp7);
            MemoryUtil.Write(m_body, len * m_rowSizeInBytes + m_offsetT8, comp8);
        }
